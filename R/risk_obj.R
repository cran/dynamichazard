...
#' \tabular{ll}{
#' \code{risk_sets}\verb{ }\tab List of lists with one for each bin. Each of the sub lists have indices that corresponds to the entries of \code{Y} that are at risk in the bin \cr
...
#' \code{is_event_in}\verb{ }\tab Indices for which bin an observation \code{Y} is an event. \code{-1} if the individual does not die in any of the bins \cr
